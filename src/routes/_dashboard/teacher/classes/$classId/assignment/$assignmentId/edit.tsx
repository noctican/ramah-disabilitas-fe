...
import { getFetcher, putFetcher } from '@/lib/api-client'
import { ASSIGNMENT, COURSE } from '@/data/const/api_path'

export const Route = createFileRoute('/_dashboard/teacher/classes/$classId/assignment/$assignmentId/edit')({
...
function EditAssignmentPage() {
  ...
  useEffect(() => {
      if (assignmentData?.data) {
          ...
          setFormData({
              title: data.title,
              description: data.description || data.instruction || '',
              points: data.max_points,
              dueDate: dateStr,
              dueTime: timeStr,
              moduleId: data.module_id || '',
              allowLate: data.allow_late !== undefined ? data.allow_late : true,
              allowFile: data.allow_file !== undefined ? data.allow_file : true,
              allowText: data.allow_text !== undefined ? data.allow_text : false
          })
      }
  }, [assignmentData])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
          ...
          await putFetcher(ASSIGNMENT.UPDATE.replace('{assignment_id}', assignmentId), { arg: payload })
          
          toast.success('Tugas berhasil diperbarui')
          // Navigate back to detail page
          navigate({ to: `/teacher/classes/${classId}/assignment/${assignmentId}` })
      } catch (error) {
          console.error(error)
    ...
    <div className="flex-1 flex flex-col min-w-0 bg-[#ffffff] dark:bg-[#22262a] font-sans text-[#121617] dark:text-white">
        ...
        <header className="h-16 border-b border-[#f1f3f4] dark:border-gray-700 bg-white dark:bg-[#22262a] px-4 sm:px-8 flex items-center justify-between sticky top-0 z-10 shrink-0">
            <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-[#677c83]">/</span>
                <Link to="/teacher/classes/$classId/assignment/$assignmentId" params={{ classId, assignmentId }} className="text-[#677c83] hover:text-primary transition-colors">Detail Tugas</Link>
                <span className="text-[#677c83]">/</span>
                <span className="font-semibold text-[#121617] dark:text-white">Edit Tugas</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="p-4 sm:p-8 max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div>
                        <h2 className="text-3xl font-black tracking-tight mb-6">Edit Tugas</h2>
